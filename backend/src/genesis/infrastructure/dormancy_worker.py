"""Nightly dormancy cycle: runs the job for every active tenant.
...
composes sessions (the export_worker injection pattern, reuse-first).

Fail-closed per tenant: a tenant whose
...
Per-tenant isolation for UNEXPECTED failures (review):
...
(reliability): the failed tenant's members are simply retried by the next
nightly cycle (the job is idempotent by construction).
...
class DormancyCycleSummary:
    ...
    #: the log records (no member data — least disclosure).
    refused: int
    #: Tenants whose run raised an UNEXPECTED error: logged
...
async def run_dormancy_cycle(
    ...
    for tenant_id in await list_active_tenants(factory):
        ...
        try:
            ...
            # the 409-class loud refusal — zero transitions for
            ...
            #  review: per-tenant isolation for
