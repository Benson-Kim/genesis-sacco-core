...
and penalty-on-arrears accrual (P13.8).
...
Penalty accrual (P13.8) extends this same job path — same scan, same
loan row lock, same short batch transactions:

  * Config comes EXCLUSIVELY from the P13.7 tenant settings
    ...
    closed); corrupted stored values raise 409, the P13.7 precedent.
  ...
  * The basis is reconstructed from the SCHEDULE and its repayment
    ...
    penalty (v1.1 rule 2; failure mode 4).
  ...
  * Claim row + loans.penalty_due update + audit row commit in ONE
    ...
    (failure mode 5, the P13.7 quorum precedent).
  * Ledger boundary (documented per P13.8): penalty INCOME recognition
    stays ON RECEIPT — P10's repayment allocation posts
  ...
  * Lock ordering: the job locks LOANS ONLY (ORDER BY l.id, FOR UPDATE
    SKIP LOCKED), the exact P10 arrears pattern. loans is the terminal
...
#: Penalty configuration read (P13.8): a single PK probe of the
#: one-row-per-tenant settings table, resolved server-side once per
#: run (v1.1 rule 1). Explicit tenant predicate on top of forced RLS
#: (v1.1 rule 4). Exported for the EXPLAIN capture
...
def parse_penalty_config(
    ...
    """Fail-closed parse of the stored penalty configuration (P13.8).
    ...
    the 0017 DB CHECKs — raise ConflictError (409), the P13.7
    ...
    never a member figure (gate 1.6).
    """
...
def arrears_scan_sql(*, with_after: bool, with_penalty: bool) -> str:
    """The arrears batch scan (P10 shape, P13.8 penalty extras).
    ...
    parameter (v1.1 rule 6). Explicit tenant predicate on top of
    ...
    if with_penalty:
        penalty_cols = (
            ...
            # Basis reconstruction (v1.1 rule 2): unpaid amounts of
            ...
            # remains the authoritative guard (v1.1 rule 5).
...
class ArrearsRunResult:
    ...
    #: — the run then accrued nothing by design (fail closed, P13.8).
...
async def _classify_loan(
    ...
    *,
    ...
    """P10 classification write for one locked loan; returns 0 or 1.
...
async def _accrue_penalty(
    ...
    *,
    ...
    """Accrue the as-of day's penalty for one LOCKED loan (P13.8).
    ...
    claim = cast(
        ...
        await session.execute(
            text(
                ...
                # race, no IntegrityError mid-batch (v1.1 rule 5). The
    ...
    result = cast(
        ...
        await session.execute(
            text(
                ...
                # (defence in depth, gate 1.6 v1.1; issue #17). Unlike
...
async def _process_batch(
    ...
    *,
    ...
    Rows are read FOR UPDATE SKIP LOCKED (gate 1.4): days-past-due and
    ...
    for row in rows:
        loan_id = str(row[0])
        oldest_unpaid = row[4]
        # The single days-past-due derivation (P10, reused verbatim for
...
async def run_arrears_for_tenant(
    ...
    *,
    ...
    batch, from tenant settings only (v1.1 rule 1): a tenant without a
