...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0044 + 0047 (head 0047 exports report-vocabulary CHECK widening for transactions_ledger, shipped by the issue-#35 export-parity MR; 0044 users phone sign-in lookup index; 0043 external txn ref + search prefix index)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0044` + `0047` — head `0047` (`0047_exports_report_check_transactions_ledger.py`, `down_revision = "0044"`; shipped by the issue-#35 export-parity MR in the same commit as this refresh; 0045/0046 claimed by open sibling MRs), verified against `versions/` on this tree |
