<!--
  ...
  spot-check check 5).
  Migration head 0048 -> 0049 by the issue-#2 withdrawal
  business-controls MR: 0049_withdrawal_business_controls.py
  (two CHECK-bounded tenant_settings money columns + the
  withdrawal_holds notice-state table with forced RLS) ships in that
  MR (same-commit refresh per v1.2 rule 11 / spot-check check 5).
...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0049 (head 0049 withdrawal business controls: cap/threshold settings + withdrawal_holds, shipped by the issue-#2 MR; 0048 human loan/exit reference columns; 0044 users phone sign-in lookup index)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0049` — head `0049` (`0049_withdrawal_business_controls.py`, `down_revision = "0048"`; shipped by the issue-#2 withdrawal business-controls MR in the same commit as this refresh), verified against `versions/` on this tree |
