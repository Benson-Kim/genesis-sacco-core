<!--
  ...
  Migration head 0048 -> 0049 by the issue-#8 approval-engine MR
  (ADR-0008): 0049_approval_engine.py (expand-only: approval_band_sets
  + pending_approvals, both forced-RLS, + transactions.checked_by;
  down_revision '0048' at branch time - re-chains per issue-#22
  governance if a sibling 0049 claim merges first) ships in that MR
...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0049 (head 0049 ADR-0008 approval engine - band sets, pending approvals, checked_by, shipped by the issue-#8 MR; 0048 human loan/exit reference columns; 0044 users phone sign-in lookup index)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0049` — head `0049` (`0049_approval_engine.py`, `down_revision = "0048"` at branch time; shipped by the issue-#8 approval-engine MR in the same commit as this refresh), verified against `versions/` on this tree |
