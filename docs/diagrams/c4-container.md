...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0049` — head `0049` (`0049_registry_scan_policies.py`, `down_revision = "0048"`; shipped by the registry-scan MR in the same commit as this refresh), verified against `versions/` on this tree |
