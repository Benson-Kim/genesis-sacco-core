<!--
  ...
  Migration head 0048 -> 0049 by the issue-#9 EOD reconciliation MR:
  0049_external_reconciliation.py (expand-only: three new FORCED-RLS
  tables recon_statements / recon_statement_lines / recon_breaks +
  idx_txns_channel_time on transactions; down_revision "0048")
  ships in that MR (same-commit refresh per v1.2 rule 11 /
  spot-check check 5).
...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0049 (head 0049 external reconciliation tables, shipped by the issue-#9 EOD reconciliation MR; 0048 human loan/exit reference columns; 0043 external txn ref + search prefix index)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0049` — head `0049` (`0049_external_reconciliation.py`, `down_revision = "0048"`; shipped by the issue-#9 EOD reconciliation MR in the same commit as this refresh), verified against `versions/` on this tree |
