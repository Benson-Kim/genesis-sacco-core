...
flowchart TB
    ...
    PG[("PostgreSQL 16 — FORCED RLS on every tenant table<br/>ADR-0002; genesis/infrastructure/tenancy.py<br/>alembic head 0046")]
...
| PostgreSQL 16, forced RLS | as-built | RLS enabled AND forced per ADR-0002 (`docs/adr/`), session scoping `genesis/infrastructure/tenancy.py` (`tenant_session` L12); migration head `0046` (`backend/migrations/versions/0046_kyc_profile_phone_e164_backfill.py`; `down_revision = "0045"` — the KYC profile-phone E.164 data backfill) |
