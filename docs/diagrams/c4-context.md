...
| PostgreSQL 16, forced RLS | as-built | RLS enabled AND forced per ADR-0002 (`docs/adr/`), session scoping `genesis/infrastructure/tenancy.py` (`tenant_session` L12); migration head `0049` (`backend/migrations/versions/0049_external_reconciliation.py`; `down_revision = "0048"` — the issue-#9 EOD reconciliation tables `recon_statements`/`recon_statement_lines`/`recon_breaks`, RLS enabled AND forced, shipped by the same MR as this refresh) |
