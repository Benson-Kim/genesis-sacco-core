...
| PostgreSQL 16, forced RLS | as-built | RLS enabled AND forced per ADR-0002 (`docs/adr/`), session scoping `genesis/infrastructure/tenancy.py` (`tenant_session` L12); migration head `0049` (`backend/migrations/versions/0049_approval_engine.py`; `down_revision = "0048"` at branch time — the ADR-0008 approval engine: `approval_band_sets` + `pending_approvals` + `transactions.checked_by`; re-chains per issue-#22 governance if a sibling 0049 claim merges first) |
