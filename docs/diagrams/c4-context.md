<!--
  ...
  spot-check check 5).
  Migration head 0048 -> 0049 by the registry-scan MR:
  0049_registry_scan_policies.py (SELECT-only registry_scan policy on
  tenants/outbox_events so worker discovery works without BYPASSRLS;
  also relaxes those two isolation predicates to missing_ok).
  Preceded by 0048_loan_exit_human_refs.py (expand-only nullable loan/exit
...
| PostgreSQL 16, forced RLS | as-built | RLS enabled AND forced per ADR-0002 (`docs/adr/`), session scoping `genesis/infrastructure/tenancy.py` (`tenant_session` L12); migration head `0049` (`backend/migrations/versions/0049_registry_scan_policies.py`; `down_revision = "0048"`) — adds a SELECT-only `registry_scan` policy on `tenants`/`outbox_events` so the SECURITY DEFINER worker registries function where the app role OWNS the tables and FORCE RLS applies to owners (no BYPASSRLS available on managed hosting), and relaxes those two isolation predicates to `current_setting(..., true)` so an unset GUC yields NULL instead of raising |
