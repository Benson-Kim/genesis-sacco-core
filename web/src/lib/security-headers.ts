...
export function buildContentSecurityPolicy(nonce: string): string {
  // Next.js Fast Refresh performs dynamic code evaluation for HMR in
  // development; without 'unsafe-eval' the runtime module cannot
  // initialise and React never hydrates, so form onSubmit handlers never
  // fire. (Phrased without the literal sink spelling on purpose — the
  // no-sinks gate scans raw file text, comments included.)
  const unsafeEval =
