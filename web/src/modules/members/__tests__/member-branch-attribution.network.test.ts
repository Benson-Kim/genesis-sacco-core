 ...
 *
 * Network-layer proofs for the member branch-attribution expand (#31
 * ledger (j).2) through the REAL generated client + middleware (node
 * environment; fetch stubbed at the network boundary — the reference
 * harness):
 * - branch_id is NULLABLE, NOT OPTIONAL: a member row MISSING the key
 *   is a contract violation and is REJECTED at the Zod boundary — the
 *   falsifiable proof that the expand landed everywhere, on the flat
 *   list, the detail read AND the opted-in aggregates rows.
 * - A set branch_id round-trips VERBATIM as the bare branch UUID; the
 *   batch-3 aggregates object rides unchanged next to it.
 */
...
let omitBranchIdOnList = false;
let omitBranchIdOnDetail = false;
...
async function fetchStub(input: Request | string | URL, init?: RequestInit): Promise<Response> {
  ...
  if (url.pathname === "/members" && request.method === "GET") {
    const row = omitBranchIdOnList ? withoutBranchId(memberOut) : memberOut;
  ...
  if (url.pathname === `/members/${MEMBER_ID}` && request.method === "GET") {
    const row = omitBranchIdOnDetail ? withoutBranchId(memberOut) : memberOut;
...
beforeEach(() => {
  calls.length = 0;
  omitBranchIdOnList = false;
  omitBranchIdOnDetail = false;
...
test("branch_id round-trips VERBATIM on the flat list, the aggregates rows AND the detail read — the batch-3 aggregates object rides unchanged", async () => {
  const flat = await membersApi.fetchMembersPage({ status: "", type: "" }, null);
  expect(flat.items[0]?.branch_id).toBe(BRANCH_ID);

  const detailRows = await membersApi.fetchMembersPageWithAggregates(
    { status: "", type: "" },
    null,
  );
  expect(detailRows.items[0]?.branch_id).toBe(BRANCH_ID);
  expect(detailRows.items[0]?.aggregates).toEqual(aggregates);

  const detail = await membersApi.fetchMemberDetail(MEMBER_ID);
  expect(detail.branch_id).toBe(BRANCH_ID);
  expect(detail.aggregates).toEqual(aggregates);
});

test("NULLABLE, NOT OPTIONAL: a list row MISSING the branch_id key is a contract violation — REJECTED at the boundary", async () => {
  omitBranchIdOnList = true;
...
test("NULLABLE, NOT OPTIONAL: a detail read MISSING the branch_id key is REJECTED at the boundary", async () => {
  omitBranchIdOnDetail = true;
